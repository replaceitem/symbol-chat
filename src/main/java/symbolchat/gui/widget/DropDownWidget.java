...
public class DropDownWidget<T> extends ClickableWidget implements Drawable, Element, Narratable {
    ...
    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        if (this.visible) {
            fill(matrices, this.x, this.y, this.x + this.width, this.y + this.height, SymbolChat.config.button_color);
            ...
            if(this.expanded) {
                fill(matrices, this.x, this.y+this.height, this.x + this.width, this.y + this.height + 1 + this.elements.size()*(this.height+1), SymbolChat.config.hud_color);
